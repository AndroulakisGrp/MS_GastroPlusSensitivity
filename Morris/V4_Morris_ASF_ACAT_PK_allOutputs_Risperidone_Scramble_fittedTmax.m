...
[values,names] = xlsread('input_parameters_v4.xlsx',4);
...
MATLABfile='Morris_Risperidone_r20_fittedTmax.mat';
...
%Update ASF Model Coefficients, C1-C4
    phystest_A(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(base_points(i,53))));
    phystest_A(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(base_points(i,54))));
    phystest_A(11)=cellstr(sprintf('C3Alpha =  %s ',num2str(base_points(i,55))));
    phystest_A(12)=cellstr(sprintf('C4Alpha =  %s ',num2str(base_points(i,56))));
...
%Update PK compartment parameters in GastroPlus for Base Matrix
    ...
    K13 = num2str(base_points(i,51)); %1/h
    K31 = num2str(base_points(i,52)); %1/h
    Renal_clearance = nah; %L/h/kg
    FPE_intestinal = nah; % %
    FPE_liver = num2str(base_points(i,57)); % %
...
%Output: Cp-time
    ...
    dataA=interp1(Time_A,Conc_A,Time,'spline');
...
for i=5%:num_of_factors
 ...
 %Update ASF Model Coefficients, C1-C4
    phystest_EE(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(temp(j,53))));
    phystest_EE(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(temp(j,54))));
    phystest_EE(11)=cellstr(sprintf('C3Alpha =  %s ',num2str(temp(j,55))));
    phystest_EE(12)=cellstr(sprintf('C4Alpha =  %s ',num2str(temp(j,56))));
...
%Save ACAT model files for Base and Auxiliary Matrix
 ...
 %Update PK compartment parameters in GastroPlus for resampling matrices
    ...
    K13 = num2str(temp(j,51)); %1/h
    K31 = num2str(temp(j,52)); %1/h
    Renal_clearance = nah; %L/h/kg
    FPE_intestinal = nah; % %
    FPE_liver = num2str(temp(j,57)); % %
    a{5} = [Body_weight, '+',  Blood_Plasma_Conc_Ratio, '+', Exp_Plasma_Fup, '+', CL, '+', Vc, '+', K12, '+', K21, '+', K13, '+', K31, '+', Renal_clearance, '+', FPE_intestinal, '+', FPE_liver];
