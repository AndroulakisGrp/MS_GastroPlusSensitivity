...
%Selected factors based on the Morris Method for Atenolol
...
%Sampled Parameters
[values,names] = xlsread('input_parameters_Sobol.xlsx',8);
...
for i=3664:repetition_of_sampling
...
%     phystest_B(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(base_points(i,14))));
    phystest_B(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(base_points(i,7))));
...
%     phystest_ABi(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(temp(j,14))));
    phystest_ABi(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(temp(j,7))));
...
reorder=vertcat(A(1:3,:),A(7,:),A(4:6,:));
reorder_names=vertcat(names(1:3,1),names(7,1),names(4:6,1));
...
% title('(A) Atenolol C_{max} First Order and Total Effects','FontSize',14)
set(gca,'XTickLabelRotation',0)

% %Second Order Effects - Cmax
% A = func_Sij(:,:,1);
% reorder=vertcat(A(1:3,:),A(7,:),A(4:6,:));
% reorder=horzcat(reorder(:,1:3),reorder(:,7),reorder(:,4:6));
% reorder_names=vertcat(names(1:3),names(7),names(4:6));
% figure()
% imagesc(reorder)
% colormap(parula)
% colorbar
% set(gca, 'XTick', 1:num_of_factors);
% set(gca,'Xticklabel', reorder_names,'FontSize',16);
% set(gca, 'Ytick', 1:num_of_factors);
% set(gca, 'Yticklabel', reorder_names,'FontSize',16);
% xlabel('Input Factors','FontSize',16,'FontWeight','bold');
% ylabel('Input Factors','FontSize',16,'FontWeight','bold');
% % title('(B) Atenolol C_{max} Second Order Effects','Fontsize',14)
