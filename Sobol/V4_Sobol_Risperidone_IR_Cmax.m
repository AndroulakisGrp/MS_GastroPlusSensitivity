...
%Selected factors based on the Morris Method for Risperidone
...
%Sampled Parameters
[values,names] = xlsread('input_parameters_Sobol.xlsx',6);
...
repetition_of_sampling = 3000; % number of N, should be set to 500 or larger
...
%     phystest_A(153)=cellstr(sprintf('Jejunum 1 Comp Pore Radius = %s ',num2str(base_points(i,1))));
...
%     phystest_A(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(base_points(i,7))));
...
%Update PK compartment parameters in GastroPlus for Base Matrix
    ...
    Body_weight = num2str(base_points(i,1)); %kg. Do not set this to 0, G+ will have an error and set weight to 70
    Blood_Plasma_Conc_Ratio = nah;
    Exp_Plasma_Fup = nah; % %
    CL = nah; %L/h
    Vc = num2str(base_points(i,2)); %L/kg
    K12 = nah; %1/h
    K21 = nah; %1/h
    K13 = nah; %1/h
    K31 = nah; %1/h
    ...
    FPE_intestinal = nah; % %
    FPE_liver = num2str(base_points(i,3)); % %
...
%Output: Cmax, tmax, AUC (0-t)
    ...
    save('Risperidone_N3000_Cmax.mat');
...
%     phystest_B(153)=cellstr(sprintf('Jejunum 1 Comp Pore Radius = %s ',num2str(base_points(i,1))));
...
%     phystest_B(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(base_points(i,14))));
    phystest_B(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(2.5)));
...
%Update PK compartment parameters in GastroPlus for Base Matrix
    ...
    Body_weight = num2str(auxiliary_points(i,1)); %kg. Do not set this to 0, G+ will have an error and set weight to 70
    Blood_Plasma_Conc_Ratio = nah;
    Exp_Plasma_Fup = nah; % %
    CL = nah; %L/h
    Vc = num2str(auxiliary_points(i,2)); %L/kg
    K12 = nah; %1/h
    K21 = nah; %1/h
    K13 = nah; %1/h
    K31 = nah; %1/h
    Renal_clearance = nah; %L/h/kg
    FPE_intestinal = nah; % %
    FPE_liver = num2str(auxiliary_points(i,3)); % %
    a{5} = [Body_weight, '+',  Blood_Plasma_Conc_Ratio, '+', Exp_Plasma_Fup, '+', CL, '+', Vc, '+', K12, '+', K21, '+', K13, '+', K31, '+', Renal_clearance, '+', FPE_intestinal, '+', FPE_liver];
...
%Output: Cmax, tmax, AUC (0-t)
    ...
    save('Risperidone_N4000_Cmax.mat');
...
for i=3:num_of_factors

    temp=base_points;
    temp(:,i)=auxiliary_points(:,i);

    for j=1918:repetition_of_sampling
...
%     phystest_ABi(153)=cellstr(sprintf('Jejunum 1 Comp Pore Radius = %s ',num2str(temp(j,1))));
...
%     phystest_ABi(9)=cellstr(sprintf('C1Alpha =  %s ',num2str(temp(j,14))));
    phystest_ABi(10)=cellstr(sprintf('C2Alpha =  %s ',num2str(2.5)));
...
%Save ACAT model files for Base and Auxiliary Matrix
 ...
 %Update PK compartment parameters in GastroPlus for resampling matrices
        ...
        nah = '54875';
        %Last entered values used for parameters set to 'nah'
    Body_weight = num2str(temp(j,1)); %kg. Do not set this to 0, G+ will have an error and set weight to 70
    Blood_Plasma_Conc_Ratio = nah;
    Exp_Plasma_Fup = nah; % %
    CL = nah; %L/h
    Vc = num2str(temp(j,2)); %L/kg
    K12 = nah; %1/
    K21 = nah; %1/h
    K13 = nah; %1/h
    K31 = nah; %1/h
...
% title('(A) Risperidone C_{max} First Order and Total Effects','FontSize',14)
...
% set(gca,'Xticklabel', names,'FontSize',16);
% set(gca, 'Ytick', 1:num_of_factors);
% set(gca, 'Yticklabel', names,'FontSize',16);
% xlabel('Input Factors','FontSize',16,'FontWeight','bold');
% ylabel('Input Factors','FontSize',16,'FontWeight','bold');
% % title('(B) Risperidone C_{max} Second Order Effects','Fontsize',14)
...
for i=1:num_of_factors
    ...
    rng default
    [bootstat_Si,bootsam_Si]=bootstrp(10000,@func_Si,func_temp,func_aux,func_base,repetition_of_sampling);    
    se_Si(i,j) = std(bootstat_Si)/sqrt(repetition_of_sampling);

    [bootstat_Sti,bootsam_Sti]=bootstrp(10000,@func_Sti_bootstrap,func_temp,func_base,repetition_of_sampling);    
    se_Sti(i,j) = std(bootstat_Sti)/sqrt(repetition_of_sampling);
    
        for k=1:num_of_factors       
        func_temp_j=func_simu_temp(:,k,j);
        [bootstat_Sij,bootsam_Sij]=bootstrp(10000,@func_Sij_bootstrap,func_temp_i,func_temp_j,func_base,repetition_of_sampling);    
